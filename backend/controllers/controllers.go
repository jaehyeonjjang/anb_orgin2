...
func (c *Controller) Display(filename string) {
	...
	//var view = jet.NewHTMLSet("C:\\\\Program Files (x86)\\ANB\\views")
	//log.Println(view)
	//log.Println(strings.Replace(filename, "/", "\\", -1))
...
func (c *Controller) Postf(name string) float64 {
	return global.Atof(c.Context.PostForm(name))
}
...
func (c *Controller) Queryf(name string) float64 {
	return global.Atof(c.Context.Query(name))
}
...
func (c *Controller) Download(filename string, downloadFilename string) {
	...
	log.Printf("download filename = %v\n", downloadFilename)
...
func (c *Controller) GetUpload(name string) string {
	...
	t := time.Now()
	//filename := fmt.Sprintf("%04d%02d%02d%02d%02d%02d_%d%v", t.Year(), t.Month(), t.Day(), t.Hour(), t.Minute(), t.Second(), t.Nanosecond(), filepath.Ext(file.Filename))
	date := fmt.Sprintf("%04d%02d%02d%02d%02d%02d_%d", t.Year(), t.Month(), t.Day(), t.Hour(), t.Minute(), t.Second(), t.Nanosecond())
	filename := date + filepath.Ext(file.Filename)
	filename2 := date + ".png"

	fullFilename := path.Join(config.UploadPath, filename)
	fullFilename2 := path.Join(config.UploadPath, filename2)

	c.Context.SaveUploadedFile(file, fullFilename)

	log.Printf("file extention = %v\n", filepath.Ext(file.Filename))
	if filepath.Ext(file.Filename) == ".pdf" {
		cmd := fmt.Sprintf("%v %v %v", config.Convert, fullFilename, fullFilename2)
		c := exec.Command("sh", "-c", cmd)
		c.Run()

		fullFilename = fullFilename2
	} else if filepath.Ext(file.Filename) == ".tif" {
		cmd := fmt.Sprintf("%v %v %v", config.Convert, fullFilename, fullFilename2)
		c := exec.Command("sh", "-c", cmd)
		c.Run()

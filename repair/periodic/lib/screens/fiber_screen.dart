...
class FiberScreen extends CWidget {
  FiberScreen({super.key});
  ...
  makeRow(int index, Point item, context) {
    ...
    var text = '${item.shape}, ${item.length}';
    if (item.shape == '') {
      text = '';
    }
    ...
    return TableRow(children: [
      ...
      GestureDetector(
        ...
        onTap: () => clickDataNumber(index, 1, context),
        child: Container(
          alignment: Alignment.center,
          padding: const EdgeInsets.all(10),
      ...
      GestureDetector(
        behavior: HitTestBehavior.translucent,
        onTap: () => clickDataNumber(index, 4, context),
        child: Container(
          alignment: Alignment.center,
          padding: const EdgeInsets.all(10),
  ...
  clickNumber(text, index, pos) {
    if (text == 'CLEAR') {
      ...
      if (c.tempText.isNotEmpty) {
        if (!c.tempText.contains('.')) {
          c.tempText = '${c.tempText}.';
        }
      } else {
        c.tempText = '0.';
      }

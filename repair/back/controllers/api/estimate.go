...
func (c *EstimateController) Insert(item *models.EstimateExtra) {
	...
	item.Estimate.Comparecount = len(item.Compares)
	err := manager.Insert(&item.Estimate)
	if err != nil {
		c.Set("code", "error")
		c.Set("error", err)
		return
	...
	for _, v := range item.Compares {
		if v.Comparecompany == 0 {
			continue
		}
		...
		err = compareestimateManager.Insert(&v)
		if err != nil {
			c.Set("code", "error")
			c.Set("error", err)
			return
...
func (c *EstimateController) Update(item *models.EstimateExtra) {
	conn := c.NewConnection()
	...
	item.Estimate.Comparecount = len(item.Compares)
	err := manager.Update(&item.Estimate)
	if err != nil {
		c.Set("code", "error")
		c.Set("error", err)
		return
	...
	err = compareestimateManager.DeleteByEstimate(item.Estimate.Id)
	if err != nil {
		c.Set("code", "error")
		c.Set("error", err)
	}
...
func (c *EstimateController) Delete(item *models.Estimate) {
	...
	err := compareestimateManager.DeleteByEstimate(item.Id)
	if err != nil {
		c.Set("code", "error")
		c.Set("error", err)
	}
...
func (c *EstimateController) Index(page int, pagesize int) {
	...
	for i, v := range items {
		compareestimates := compareestimateManager.FindByEstimate(v.Id, []interface{}{models.Ordering("e_id")})
		items[i].Extra["compareestimate"] = compareestimates
	}

...
var materialNames = []string{"오염", "파손", "박리", "박락"}
...
func (c *DataItem) Result() []DataResult {
	...
	if c.Check[5] == true {
		...
		for i, v := range c.Material {
			if v == true {
				results = append(results, materialNames[i])
			}
		}
...
func (c *DataItem) IsValid() bool {
	if len(c.Part) > 0 && len(c.Cause) > 0 {
		if slices.Contains(c.Check, true) {
			return true
		}
	}

	...
	for _, dong := range summaryDongs {
		...
		for _, blueprint := range blueprints {
			...
			for _, v := range items {
				...
				if v.Type < 1 || v.Type > 13 {
					continue
				}

				position := 3
				switch blueprint.Floortype {
				case 4, 5:
					position = 1
				case 1, 2:
					position = 5
		...
		for i := range floorCount {
			...
			for _, data := range summarys[i].Data {
				...
				if strings.Contains(data.Shape, "마감재") {
					for i, v := range materialNames {
						if strings.Contains(data.Shape, v) {
							material[i] = true
							break
						}
					}
				...
				if strings.Contains(data.Shape, "누수") {
					strs := strings.SplitSeq(data.Shape, "/")
					for name := range strs {
						switch name {
						case "누수":
							leak1 = true
				...
				for j := range 6 {
					...
					find = slices.Contains(causes[j], data.Remark)
			...
			for j := range floorCount {
				...
				case 5:
					for i, v := range material {
						if v == true {
							titles = append(titles, materialNames[i])
						}
					}
...

		{
			...
			for i := 0; i <= 3; i++ {
				...
				if check[i*2] == true || check[i*2+1] == true {
					...
					for _, v := range items[i] {
						results := strings.SplitSeq(v.Item.Result, "/")

						for v2 := range results {
							switch v2 {
							case "균열":
								leak1 = true
							case "누수":
...

		{
			...
			for i := 0; i <= 3; i++ {
				if check[i*2] == true || check[i*2+1] == true {
					...
					for _, v := range items[i] {
						for i, v2 := range materialNames {
							if strings.Contains(v.Item.Result, v2) {
								material[i] = true
								break
							}
						}
					...
					for i, v := range material {
						if v == true {
							temp = append(temp, materialNames[i])
						}
					}

					materialStr := fmt.Sprintf("마감재 %v", strings.Join(temp, "/"))
					...
					item.Material = append(item.Material, fmt.Sprintf("%v - %v", title, materialStr))
				}
			}
		}
		...
		for i := range 4 {
			if check[i*2] == true || check[i*2+1] == true {
				...
				for _, v := range items[i] {
					temp := strings.SplitSeq(v.Item.Part, ", ")
					for v2 := range temp {
						find := slices.Contains(parts, v2)
				...
				if i == 3 {
					...
					if other.Head != "" {
						...
						resultItems = append(resultItems, fmt.Sprintf("%v %v 마감부위에서 %v", numbers[numberPos], outerwallMaterial, strings.Join(other.Items, ", ")))
						numberPos++
					}
				}
			}

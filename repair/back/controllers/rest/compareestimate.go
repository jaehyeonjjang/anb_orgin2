...
type CompareestimateController struct {
	controllers.Controller
}



func (c *CompareestimateController) FindByEstimate(estimate int64) []models.Compareestimate {
    
    conn := c.NewConnection()

	_manager := models.NewCompareestimateManager(conn)
    
    item := _manager.FindByEstimate(estimate)
...
// @Delete()
func (c *CompareestimateController) DeleteByEstimate(estimate int64) {
    
    conn := c.NewConnection()

	_manager := models.NewCompareestimateManager(conn)
    
    err := _manager.DeleteByEstimate(estimate)
    if err != nil {
        c.Set("code", "error")    
        c.Set("error", err)
        return
...
func (c *CompareestimateController) Insert(item *models.Compareestimate) {
    
    
	conn := c.NewConnection()
    
	manager := models.NewCompareestimateManager(conn)
...
func (c *CompareestimateController) Insertbatch(item *[]models.Compareestimate) {  
...
func (c *CompareestimateController) Update(item *models.Compareestimate) {
...
func (c *CompareestimateController) Delete(item *models.Compareestimate) {
...
func (c *CompareestimateController) Deletebatch(item *[]models.Compareestimate) {
...
func (c *CompareestimateController) Count() {
    ...
    _comparecompany := c.Geti64("comparecompany")
    if _comparecompany != 0 {
        args = append(args, models.Where{Column:"comparecompany", Value:_comparecompany, Compare:"="})    
    }
...
func (c *CompareestimateController) Read(id int64) {
...
func (c *CompareestimateController) Index(page int, pagesize int) {
...
func (c *CompareestimateController) Sum() {

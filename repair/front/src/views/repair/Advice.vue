...
import { Repair, Advice } from "~/models"
...
function clickDelete() {
  ...
  util.confirm('삭제하시겠습니까', async function() {
    let res = await Advice.remove(item)
    if (res.code === 'ok') {
      util.info('삭제되었습니다')
      data.visible = false
      getItems()
...
onMounted(async () => {
  ...
  data.repair = parseInt(route.params.id)
...
async function clickSubmit() {
  ...
  for (let i = 0; i < data.batchs.length; i++) {
    ...
    if (item.id > 0) {
      await Advice.update(item)
    } else { 
      await Advice.insert(item)
    }
  }

<template>
  ...
  <div style="display:flex;gap: 10px;margin-bottom:10px;">
    <el-select v-model.number="data.search.status" placeholder="상태" style="width:80px;">           
      <el-option
        v-for="(item, index) in Detail.statuss"
        :key="index"
        :label="item"
        :value="index"
      />
  ...
  <el-table :data="data.items" border style="width: 100%;" :height="height(170)" v-infinite="getItems" @row-click="clickUpdate" >
    ...
    <el-table-column label="상태" align="center" width="60">
      <template #default="scope">
        {{Detail.getStatus(scope.row.status)}}
      </template>
    </el-table-column>
    <el-table-column prop="date" label="등록일" align="center" width="130" />
    <el-table-column label="" width="80" align="center" >
      <template #default="scope">        
        <el-button size="small" type="success" @click="clickDetail(scope.row)">정밀점검</el-button>
      </template>
    </el-table-column>
  </el-table>  
      ...
      <div style="display:flex;flex-direction:row;">
        ...
        <div style="flex:1 1 67%;">
          ...
          <el-form-item label="장기수선계획" style="margin:5px 0px;">
            <el-input v-model="data.item.detail" :rows="5" type="textarea" />
          </el-form-item>
  ...
  <el-dialog v-model="data.visibleDetail" width="800px" :before-close="handleClose" >
    <DetailInsert :id="data.id" ref="detail" />
  </el-dialog>
...
async function getItems(reset: boolean) {
  ...
  let res = await Detail.find({page: data.page, pagesize: data.pagesize, status: data.search.status, search: data.search.text})
...
async function clickDetail(item) {
  const apt = item.apt
  const id = item.id
  router.push(`/${apt}/detail/${id}/detail`)
}

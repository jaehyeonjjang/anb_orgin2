...
import { Reviewcontentbasic } from "~/models"
...
function clickDelete() {
  ...
  util.confirm('삭제하시겠습니까', async function() {
    let res = await Reviewcontentbasic.remove(item)
    if (res.code === 'ok') {
      util.info('삭제되었습니다')
      data.visible = false
      getItems()
...
function clickDeleteMulti() {
  util.confirm('삭제하시겠습니까', async function() {
    ...
    for (let i = 0; i < listSelection.value.length; i++) {
      ...
      await Reviewcontentbasic.remove(item)
    }
...
async function clickSubmit() {
  ...
  for (let i = 0; i < data.batchs.length; i++) {
    ...
    if (item.id > 0) {
      await Reviewcontentbasic.update(item)
    } else { 
      await Reviewcontentbasic.insert(item)
    }
  }

<template>
  ...
  <div style="display:flex;gap: 10px;margin-bottom:10px;">
    <el-select v-model.number="data.search.status" placeholder="상태" style="width:80px;">           
      <el-option
        v-for="(item, index) in Periodic.statuss"
        :key="index"
        :label="item"
        :value="index"
      />
  ...
  <el-table :data="data.items" border style="width: 100%;" :height="height(170)" v-infinite="getItems" @row-click="clickUpdate" >
    ...
    <el-table-column label="상태" align="center" width="60">
      <template #default="scope">
        {{Periodic.getStatus(scope.row.status)}}
      </template>
    </el-table-column>
    <el-table-column prop="date" label="등록일" align="center" width="130" />
    <el-table-column label="" width="80" align="center" >
      <template #default="scope">        
        <el-button size="small" type="success" @click="clickPeriodic(scope.row)">정기점검</el-button>
      </template>
    </el-table-column>
  </el-table>  
  ...
  >
    <el-form :model="data.item" label-width="100px">
      ...
      <div style="display:flex;flex-direction:row;">
        ...
        <div style="flex:1 1 67%;">
          ...
          <el-form-item label="장기수선계획" style="margin:5px 0px;">
            <el-input v-model="data.item.periodic" :rows="5" type="textarea" />
          </el-form-item>
      ...
      <div style="display:flex;flex-direction:row;">
        <div style="flex:1 1 33%;">
          <el-form-item label="정기점검일자" style="margin:5px 0px;">
  ...
  <el-dialog v-model="data.visiblePeriodic" width="800px" :before-close="handleClose" >
    <PeriodicInsert :id="data.id" ref="periodic" />
  </el-dialog>

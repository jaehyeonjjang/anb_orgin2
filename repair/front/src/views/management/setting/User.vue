<template>
  ...
  <el-table :data="data.items" border style="width: 100%;" :height="height(200)" v-infinite="getItems">
    ...
    <el-table-column prop="level" width="130" label="권한" align="center">
...
const data = reactive({
  ...
  visible: false,
  levels: [{id: 0, name:'권한'}, {id: 3, name:'사이트 관리자'}, {id:4, name:'사이트 총관리자'}]
})
...
async function getItems(reset) {
  ...
  let res = await User.find({page: data.page, name: search.text})

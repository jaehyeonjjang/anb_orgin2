<template>
  ...
  <y-table>
    <y-tr>
      <y-th>최고층고</y-th>
      <y-td>부위 : {{data.periodic.struct1}}, 층고 : {{data.periodic.struct2}}</y-td>
    </y-tr>
    <y-tr>
      <y-th>기둥최장간격</y-th>
      <y-td>{{data.periodic.struct3}} × {{data.periodic.struct4}}</y-td>
    </y-tr>
    <y-tr>
      <y-th>지정형식</y-th>
      <y-td>{{data.struct5s[data.periodic.struct5]}}</y-td>
    </y-tr>    
    <y-tr>
      <y-th>PILE․PIER의 근입심도</y-th>
      <y-td>{{data.periodic.struct6}}</y-td>
    </y-tr>
    <y-tr>
      <y-th>PILE의 지지방법</y-th>
      <y-td>{{data.periodic.struct7}}</y-td>
    </y-tr>
    <y-tr>
      <y-th>강재종류</y-th>
      <y-td>{{data.periodic.struct8}}</y-td>
    </y-tr>
    <y-tr>
      <y-th>외벽 주요 마감자재</y-th>
      <y-td>{{data.periodic.struct9}}</y-td>
    </y-tr>
    <y-tr>
      <y-th>실내바닥 마감자재</y-th>
      <y-td>{{data.periodic.struct10}}</y-td>
    </y-tr>
    <y-tr>
      <y-th>실내벽체 마감자재</y-th>
      <y-td>{{data.periodic.struct11}}</y-td>
    </y-tr>
  </y-table>
...
import { Aptperiodic } from "~/models"

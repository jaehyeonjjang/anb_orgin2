  ...
  >
    ...
    <y-table>
      ...
      <y-tr v-for="(item, index) in data.batchs" :key="item.id">
        ...
        <y-td>
          <div style="display:flex;" v-if="item.type != 1 && item.type != 3">
          ...
          <div style="display:flex;" v-else>
            <el-radio-group v-model="data.batchs[index].status">
              <el-radio  v-for="(item, pos) in getInput(item.name, item.status)" :label="item.name"><span style="font-size:12px;">{{item.name}}</span></el-radio>                
            </el-radio-group>
          </div>
        </y-td>

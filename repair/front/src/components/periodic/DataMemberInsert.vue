<template>
  ...
  <el-table :data="data.items" border :height="height(170)" @row-click="clickUpdate"  ref="listRef" @selection-change="changeList">
    ...
    <el-table-column prop="group" label="측정위치" align="center" width="70" />
    <el-table-column prop="part" label="도면표기 (mm)" align="center" width="140" />
    <el-table-column prop="member" label="측정치수 (mm)" align="center" width="140" />
    <el-table-column prop="shape" label="마감재 유/무" align="center" width="120" />
  ...
  >
    ...
    <el-form>      
      <el-table :data="data.batchs" border table-layout="fixed" style="margin-top:15px;width:640px;">
        ...
        <el-table-column label="측정위치" align="center" width="60">
          <template #default="scope">
            <el-input v-model.number="data.batchs[scope.$index].group" />
          </template>
        </el-table-column>
        <el-table-column label="도면표기 (mm)" align="center" width="130">
          <template #default="scope">
            <el-input v-model="data.batchs[scope.$index].part" />
          </template>
        </el-table-column>
        <el-table-column label="측정치수 (mm)" align="center" width="130">
          <template #default="scope">
            <el-input v-model="data.batchs[scope.$index].member" />
          </template>
        </el-table-column>
        <el-table-column label="마감재 유/무" align="center" width="130">
          <template #default="scope">
            <el-select v-model="data.batchs[scope.$index].shape" style="width:100%;">
...
async function getItems() {
  ...
  for (let i = 0; i < res.items.length; i++) {
    ...
    if (item.type < 400 || item.type >= 500) {
      continue
    }

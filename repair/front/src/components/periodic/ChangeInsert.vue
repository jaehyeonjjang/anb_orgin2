<template>
  <div style="display:flex;justify-content:space-between;margin-bottom:10px;">
    <div></div>
    <el-button size="small" type="success" @click="clickChange(2)" v-if="data.periodic.category == 1">정밀점검으로 전환</el-button>
    <el-button size="small" type="success" @click="clickChange(1)" v-if="data.periodic.category == 2">정기점검으로 전환</el-button>
  </div>
...
async function clickChange(pos) {
  const titles = ['', '정기점검', '정밀점검']
  util.confirm(`${titles[pos]}으로 전환하시겠습니까`, async function() {
    let res = await Periodic.get(data.id)

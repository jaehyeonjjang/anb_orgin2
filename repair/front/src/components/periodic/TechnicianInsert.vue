<template>
  ...
  <el-table :data="data.items" border :height="height(224)">
    <el-table-column prop="type" label="참여구분" align="center" width="100">
      <template #default="scope">
        {{Periodictechnician.getType(scope.row.type)}}
      </template>
    </el-table-column>
    <el-table-column prop="extra.technician.name" label="성명" align="center" width="80" />
    <el-table-column prop="part" label="참여분야" />
    <el-table-column prop="part" label="참여 기간" align="center" width="240">
      <template #default="scope">
      ...
      <y-tr>
        ...
        <y-td>
          <el-radio-group v-model.number="data.item.type">
            <el-radio-button size="small" label="1" @click="clickType(1)">책임기술자</el-radio-button>
            <el-radio-button size="small" label="2" @click="clickType(2)">참여기술자</el-radio-button>
          </el-radio-group>
        </y-td>
      </y-tr>
...
import { Periodictechnician, Technician } from "~/models"
...
async function getItems() {
  let res = await Periodictechnician.find({periodic: data.id, orderby: 'dt_order,dt_id'})
...
async function clickSubmit(type) {
  ...
  if (item.id > 0) {
    await Periodictechnician.update(item)
    util.info('수정되었습니다')
  } else {
    let max = 0
    ...
    item.order = max + 1
    await Periodictechnician.insert(item)
    util.info('등록되었습니다')
  }
...
function clickDelete(item, index) {
  util.confirm('삭제하시겠습니까', async function() {
    let res = await Periodictechnician.remove(item)
    if (res.code === 'ok') {
      util.info('삭제되었습니다')
      await getItems(true)
    }
...
async function clickUp(row, index) {
  ...
  await Periodictechnician.update(items[index])
  await Periodictechnician.update(items[index - 1])

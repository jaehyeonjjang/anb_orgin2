<template>
  ...
  <el-table :data="data.items" border :height="height(224)" @row-click="clickUpdate"  ref="listRef" @selection-change="changeList">
    ...
    <el-table-column prop="floor" label="구분" align="center">
      <template #default="scope">        
        ...
        <span v-if="scope.row.type==6">안전진단전문기관 등록증</span>
        <span v-if="scope.row.type==7">책임기술자 수료증</span>
      </template>
    </el-table-column>    
        ...
        <y-tr>
          ...
          <y-td>
            <el-radio-group v-model.number="data.item.type">            
              <el-radio-button size="small" label="4">과업수행계획서 승인</el-radio-button>
              <el-radio-button size="small" label="6">안전진단전문기관 등록증</el-radio-button>
              <el-radio-button size="small" label="7">책임기술자 수료증</el-radio-button>
              <el-radio-button size="small" label="5">전회점검 결과표</el-radio-button>
            </el-radio-group>
          </y-td>
        </y-tr>        
        <y-tr>
...
async function clickSubmit() {
  if (util.getInt(data.item.type) == 0) {
    util.error('구분을 선택하세요')
    return
  }
  
  if (data.item.filename == '') {
    util.error('이미지를 선택하세요')
    return
  }
  ...
  if (item.id > 0) {
    ...
    if (util.getExt(item.filename) == 'pdf') {
      await model.process(item)
    } else {
...
async function handleRemove(uploadFile: UploadFile, uploadFiles: UploadFiles) {
  data.item.filename = ''  
}

const submitUpload = () => {
  upload.value.clearFiles()
  upload.value!.clearFiles(['success'])
  upload.value!.submit()
}
